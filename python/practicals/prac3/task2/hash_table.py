...
class LinearProbeHashTable(Generic[T]):
            ...
            if self.table[position] is None:  # found empty slot
                if is_insert:
                    if probe_current > 0:
                        ...
                        self.probe_total += probe_current
                    if probe_current > self.probe_max:
                        self.probe_max = probe_current
                    return position
                else:
    ...
    def statistics(self) -> Tuple:
        """which returns a tuple (conflict_count, probe_total, probe_max, rehash_count), 
        consisting of 
                1. the total number of conflicts,
                2. the sum of all the probe chain lengths,
                3. the length of the longest probe chain,
                4. the number of times rehash has been called.
        """
        my_tuple = (self.conflict_count, self.probe_total, self.probe_max, self.rehash_count)
        return my_tuple
