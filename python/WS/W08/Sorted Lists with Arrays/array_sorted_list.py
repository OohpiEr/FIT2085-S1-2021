...
class ArraySortedList(SortedList[T]):
    ...
    def __setitem__(self, index: int, value: T) -> None:
        ...
        """
        if self.is_empty() or (index == 0 and value <= self[index]) or \
                (index == len(self) and value >= self[index - 1]) or \
                (0 < index and self[index - 1] <= value <= self[index]):
