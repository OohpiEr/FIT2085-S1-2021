...
"""
__author__ = "Maria Garcia de la Banda for the base" + "Er Tian Ru student for __str__(self) and __getitem__(self)"
...
class ArrayStack(Stack[T]):
    ...
    def __getitem__(self, index: int) -> T:
        ...
        :pre: index in between 0 and length - self.array[]
        :complexity: O(1)
        """
        return self.array[index]

    def __str__(self) -> str:
        """returns a string containing the information of each element in stack
        ...
        if self.is_empty():
            ...
            for i in range(len(self.array) - 1, -1, -1):
                string += '{}\'s life = {} and experience = {},'.format(self[i].get_unit_type(),
                                                                        self[i].get_life(),
                                                                        self[i].get_experience())
            string = string[:-1]
